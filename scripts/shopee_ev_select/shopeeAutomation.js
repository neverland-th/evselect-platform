...
export class ShopeeAutomation {
  ...
  async initBrowser() {
    ...
    // เคลียร์ทรัพยากรเดิมก่อนเปิดใหม่เพื่อป้องกัน zombie chromium process
    await this.close();
  ...
  async navigateAndFillLogin(username, password) {
    ...
    if (usernameInput) {
      await usernameInput.fill(String(user));
    ...
    if (passwordInput) {
      await passwordInput.fill(String(pass));
      ...
      checkInterval = setInterval(async () => {
        ...
        try {
          if (this.browser && !this.browser.isConnected()) {
            resolved = true;
            cleanup();
            console.log('\nℹ️ ตรวจพบว่าเบราว์เซอร์ถูกปิดหรือตัดการเชื่อมต่อ');
  ...
  async draftInitialListings(products = productListingsData) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const keyFeatures = Array.isArray(p.keyFeaturesThai)
        ? p.keyFeaturesThai.filter(f => f != null).map(String)
        : [];
  ...
  async configureShippingAndPayment(customConfig = {}) {
    ...
    const extractCouriers = (couriers, defaultList) => {
      if (!Array.isArray(couriers)) return defaultList;
      const formatted = couriers
        .filter(c => c != null)
        .map(c => (typeof c === 'object' ? (c.name || '') : String(c)).trim())
        .filter(Boolean);
      return formatted.length > 0 ? formatted.join(', ') : defaultList;
    };

    const stdCouriers = extractCouriers(
      config.shippingChannels?.standardDelivery?.supportedCouriers,
      'SPX Express, Flash Express, Kerry Express / KEX, J&T Express'
    );
    const bulkyCouriers = extractCouriers(
      config.shippingChannels?.standardDeliveryBulky?.supportedCouriers,
    ...
    const validPaymentMethods = Array.isArray(config.paymentMethods?.acceptedCustomerPaymentMethods)
      ? config.paymentMethods.acceptedCustomerPaymentMethods.filter(m => m != null)
      : [];
    validPaymentMethods.forEach(m => {
      const methodName = typeof m === 'object' ? (m.method || '') : String(m);
      const benefit = typeof m === 'object' ? (m.benefitThai || '') : '';
      console.log(`     ✓ ${methodName}${benefit ? ` (${benefit})` : ''}`);
    });
    ...
    const validGuides = Array.isArray(config.setupGuideThai)
      ? config.setupGuideThai.filter(g => g != null)
      : [];
    validGuides.forEach(guide => {
      const stepNum = guide.stepNumber ?? '-';
      const title = guide.title || '';
      const url = guide.url || '';
      const steps = Array.isArray(guide.steps) ? guide.steps.filter(Boolean) : [];
      console.log(`\n📌 ขั้นตอนที่ ${stepNum}: ${title}`);
      if (url) console.log(`   ลิงก์ตั้งค่า: ${url}`);
