...
import BrakeKitReferences from '@/components/articles/BrakeKitReferences';
...
function Ref({ to, children }: { to: keyof typeof brakeSources; children: ReactNode }) {
  return <a href={brakeSources[to].url} target="_blank" rel="noopener noreferrer" className="font-medium text-lime-800 underline decoration-lime-300 underline-offset-4 hover:text-lime-950">{children}</a>;
}
...
const contents = [
  ['quick-choice', 'เริ่มจากอาการและเป้าหมาย'], ['energy', 'ลองคำนวณภาระเบรก'], ['disc-size', 'จานใหญ่และหนาช่วยอะไร'], ['surface', 'จานเรียบ / เซาะร่อง / เจาะรู'], ['horsepower', 'กี่แรงม้าควรใช้เบรกอะไร'], ['value', 'ลำดับอัปเกรดที่คุ้ม'], ['japan', 'เคส ENDLESS จาน 375 mm ของเจ้าของรถ'], ['complete-kits', '4 ชุดหน้าใหม่และราคาจากต้นทาง'], ['brand-compare', 'กดเทียบ Performance / ราคา / ฝุ่น'], ['carbon-ceramic', 'Carbon Ceramic สำหรับสายสุด'], ['thai-roads', 'ถนนไทยกับคำกล่าว 1,000°C'], ['temperature', 'พลังงานไม่ใช่อุณหภูมิ'], ['before-buying', 'เช็กลิสต์ก่อนซื้อ'], ['faq', 'คำถามที่คนซื้อเบรกเจอบ่อย'],
] as const;

const faqs = [
  { q: 'ล้อ 20 นิ้ว ใส่จาน 380 mm และคาลิเปอร์ 6 พอตได้ทุกวงไหม?', a: 'ไม่ได้ ขนาดนิ้วไม่บอกทรงก้านล้อ ออฟเซ็ต หรือพื้นที่ด้านใน ต้องใช้ template ของรหัสชุดกับล้อจริง และตรวจรุ่นรถ ปี ตลาด รวมถึงระยะสายเบรกและระบบควบคุม เคส Advan 20×10.5 กับจาน 375 mm ของเจ้าของเป็นข้อมูลเฉพาะคัน ไม่ใช่ใบรับรองล้ออื่น' },
  { q: 'ราคา ENDLESS ALVELL6 ใช้แทนราคา M6 ใน Tesla ได้ไหม?', a: 'ไม่ได้ เป็นคนละตระกูลและคนละ application ALVELL6 FIYIXGH40W ในตารางเป็นชุด Alphard/Vellfire 40 ส่วนฉลาก EC690/RCP209 ในภาพเจ้าของสอดคล้องกับ M6 ต้องขอใบเสนอราคาตรงรหัสใหม่ และตรวจรายการในชุดให้ครบ' },
  { q: 'กล้อง Thermal เห็นไม่ถึง 200°C แปลว่าใช้เบรก 700°C ไปไม่ถึงหนึ่งในสามหรือไม่?', a: 'ไม่ใช่ ค่าหลังหยุดอาจไม่ใช่จุดสูงสุดระหว่างเบรก และอัตราส่วนตัวเลของศาเซลเซียสไม่ใช่เปอร์เซ็นต์สมรรถนะ 700°C ที่ยกมาคือขอบเขตจานสำหรับผ้า MX72 ไม่ใช่พิกัดของทั้งชุด ใช้ผลวัดพร้อมบริบทและอาการจริงตัดสินความจำเป็นของการอัปเกรด' },
...
export default function CarbonCeramicBrakesGuidePage() {
  ...
  return <article className={`${styles.article} mx-auto max-w-5xl bg-white px-4 py-10 font-sans text-slate-900 sm:px-6 md:py-16 lg:px-8`}>
    ...
    <header className="space-y-6">
      ...
      <div className="max-w-3xl space-y-4 text-base leading-[1.95] text-slate-600 sm:text-lg">
        <p>จาน Carbon Ceramic คาลิเปอร์สวย ๆ และรายละเอียดที่มองผ่านก้านล้อ สำหรับคนรักรถ เสน่ห์ของเบรกดี ๆ เริ่มได้ตั้งแต่ยังไม่ได้ออกตัว ความอยากได้จึงไม่ใช่เรื่องแปลก แต่ก่อนจ่ายหลักแสนให้ EV คันโปรด เราควรรู้ว่าเบรกชุดนั้นจะตอบแทนเราด้วยอะไร ทั้งตอนมองรถ ตอนเหยียบเบรก และวันที่ต้องเปลี่ยนอะไหล่</p>
        <p><BrandHomeLink /> ชวนทำความรู้จัก Carbon Ceramic ทั้งเสน่ห์และสิ่งที่ต้องยอมรับ แล้วเทียบกับชุดเบรกจาก Brembo และ ENDLESS ในเรื่องสมรรถนะ ฝุ่น และค่าใช้จ่าย พร้อมประสบการณ์ ENDLESS จาน 375 mm บน <Link href="/articles/tesla-model-3-highland-review" className="text-lime-800 underline underline-offset-4">Tesla Model 3 Performance Highland</Link> เพื่อช่วยตอบว่า สำหรับรถที่คุณใช้และทางที่คุณขับ การอัปเกรดแบบไหนคุ้มกับคุณที่สุด</p>
      </div>
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-slate-500"><span className="inline-flex items-center gap-2"><CalendarDays className="h-4 w-4" aria-hidden="true" />อัปเดต <time dateTime={brakeArticle.updatedAt}>{new Intl.DateTimeFormat('th-TH', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'Asia/Bangkok' }).format(new Date(`${brakeArticle.updatedAt}T00:00:00+07:00`))}</time></span><span className="inline-flex items-center gap-2"><Clock className="h-4 w-4" aria-hidden="true" />ประมาณ {brakeArticle.readTime}</span><span><BrandHomeLink /> · เรียบเรียงจากข้อมูลผู้ผลิต</span></div>
      <figure className="overflow-hidden rounded-3xl border border-slate-200"><Image src={brakeArticle.cover} alt={brakeArticle.coverAlt} width={brakeArticle.coverWidth} height={brakeArticle.coverHeight} sizes="(max-width: 1024px) 100vw, 960px" priority className="h-auto w-full" /><figcaption className="bg-slate-50 px-5 py-4 text-sm leading-relaxed text-slate-500">จาน Carbon Ceramic PCCB และคาลิเปอร์สีเหลืองบน Porsche Carrera S (997) ขณะถอดล้อ · ภาพ: <a href={brakeArticle.coverSource} target="_blank" rel="noopener noreferrer" className="underline underline-offset-4">{brakeArticle.coverAuthor}</a> / Wikimedia Commons · <a href={brakeArticle.coverLicenseUrl} target="_blank" rel="noopener noreferrer" className="underline underline-offset-4">{brakeArticle.coverLicense}</a> · ย่อขนาดจากต้นฉบับ ภาพประกอบเทคโนโลยี ไม่ใช่ชุดที่รับรองให้ติดตั้งกับ EV ทุกรุ่น</figcaption></figure>
    </header>
    ...
    <div className="mt-10 space-y-12 text-base leading-[1.95] sm:text-lg">
      <aside className="rounded-2xl border border-lime-200 bg-lime-50 p-5 sm:p-7">
        <h2 className="mb-3 text-xl font-bold text-slate-950 sm:text-2xl">คำแนะนำของเรา: ชุดที่ตรวจที่มาได้ และตรงกับงานของรถ</h2>
        ...
        <p className="mt-3 text-sm text-slate-600">ล้อ 20 นิ้วเป็นโจทย์ตั้งต้น ไม่ใช่ใบรับรองว่าเบรกทุกชุดใส่ได้ ต้องตรวจทรงก้านล้อ ออฟเซ็ต และแบบระยะเคลียร์เฉพาะชุด นี่คือข้อเสนอเชิงบรรณาธิการตามลักษณะใช้งาน ไม่ใช่ผลทดสอบรถทุกคัน หรือคำรับรองสินค้าเฉพาะรุ่น <BrandHomeLink /> ยังไม่มีสินค้าพร้อมจำหน่าย และยังไม่เปิดรับคำสั่งซื้อหรือชำระเงิน</p>
      </aside>

      <div className="flex flex-wrap gap-3"><a href="#carbon-ceramic" className="inline-flex min-h-11 items-center rounded-xl border border-lime-600 px-4 py-2 text-base font-semibold text-lime-800">Carbon Ceramic ได้อะไร แลกกับอะไร?</a><a href="#japan" className="inline-flex min-h-11 items-center rounded-xl border border-slate-300 px-4 py-2 text-base font-semibold">ดูเคส ENDLESS 375 mm</a><a href="#brand-compare" className="inline-flex min-h-11 items-center rounded-xl bg-lime-400 px-4 py-2 text-base font-bold text-slate-950">กดเทียบ 3 แนวทางเบรก →</a></div>

      <nav aria-label="สารบัญบทความเบรก EV" className="rounded-2xl border border-slate-200 p-5 sm:p-7"><h2 className="mb-4 text-xl font-bold">เลือกอ่านจากสิ่งที่กำลังสงสัย</h2><ol className="grid gap-x-6 gap-y-1 sm:grid-cols-2">{contents.map(([id, title], index) => <li key={id}><a href={`#${id}`} className="flex min-h-11 items-start gap-3 py-2 text-base text-slate-700 hover:text-lime-800"><span className="min-w-6 shrink-0 whitespace-nowrap font-bold tabular-nums text-lime-700">{String(index + 1).padStart(2, '0')}</span>{title}</a></li>)}</ol></nav>
      ...
      <Section id="quick-choice" title="1. อยากให้เบรกดีขึ้น — ดีขึ้นด้านไหน?">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            ['ช่วงแรกดี พอเบรกซ้ำแล้วแรงลด', 'ต้องแยกผ้าเบรกเฟด อุณหภูมิเกินขอบเขต และปัญหาอื่นของระบบ ขอการตรวจสภาพและข้อมูลอุณหภูมิจากผู้เชี่ยวชาญก่อนเลือกสูตรผ้าเบรก (compound) จาน หรือช่องระบายอากาศใหม่'],
            ...
            ['ใช้งานปกติ แต่ฝุ่นเยอะหรืออยากได้งานสวย', 'แยกเป้าหมายนี้ได้เลย: ผ้าแนวฝุ่นน้อยที่เข้ากันกับจานอาจตรงกว่าชุดเบรกใหญ่ (Big Brake Kit หรือ BBK) ส่วนชุดคาลิเปอร์หรือจาน Carbon Ceramic (CCB) อาจตอบความชอบด้านงานผลิตและรูปลักษณ์ ควรบอกงบและข้อแลกเปลี่ยนที่ยอมรับได้ให้ชัด'],
      ...
      <Section id="disc-size" title="3. จานใหญ่ขึ้น หนาขึ้น มีประโยชน์ต่างกันอย่างไร?">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-slate-200 p-6"><h3 className="text-xl font-bold">ความหนา มวล และช่องลม → การรับและคายความร้อน</h3><p className="mt-4">เนื้อจานมากขึ้นช่วยรับพลังงานโดยอุณหภูมิเพิ่มช้าลงภายใต้เงื่อนไขเดียวกัน ส่วนช่องระบายอากาศภายในและอากาศที่ไหลเข้าช่วยพาความร้อนออก จานที่หนาหรือใหญ่ขึ้นอาจหนักขึ้น จึงต้องชั่งน้ำหนักชุดจริง</p><p className="mt-3 text-sm text-slate-500">แผนภาพแขนแรงนี้จัดทำโดย <BrandHomeLink /> ไม่ใช่แบบผลิตหรือภาพทดสอบชิ้นส่วน</p></div>
        </div>
        ...
        <p><strong>จำนวนพอตก็ไม่ใช่คะแนน:</strong> พื้นที่ลูกสูบรวม ความแข็งแกร่งของคาลิเปอร์ ขนาดผ้า แม่ปั๊ม และสมดุลหน้า–หลังต้องทำงานร่วมกัน การซื้อ 6 พอตเพราะมากกว่า 4 พอต โดยไม่ตรวจทั้งระบบยังตอบไม่ได้ว่าจะแก้ปัญหาเดิมอย่างไร</p>
      </Section>
      ...
      <Section id="surface" title="4. จานเรียบ เซาะร่อง เจาะรู — ลายจานบอกอะไรและไม่บอกอะไร">
        ...
        <div className="grid items-start gap-4 sm:grid-cols-2">
          <Photo src="/images/articles/slotted-disc-numobeer.jpg" alt="ภาพถ่ายจานเบรกเซาะร่อง เห็นร่องโค้งบนผิวเสียดทาน" width={1280} height={720}><strong className="text-slate-800">ร่องบนผิวจาน / Slotted</strong><br />ภาพ: <Ref to="slottedPhoto">Numobeer</Ref> · <Ref to="ccBySa4">CC BY-SA 4.0</Ref> · ภาพปี 2016 แสดงเต็มเฟรม ย่อขนาดสำหรับเว็บ ไม่ได้ระบุว่าเป็นจาน Brembo Max</Photo>
          <Photo src="/images/articles/amg-carbon-ceramic-brake-gold.jpg" alt="จาน Carbon Ceramic ผิวเจาะรูกับคาลิเปอร์ AMG สีทองในงาน Geneva Auto Show 2011" width={1920} height={1278}><strong className="text-slate-800">รูบนผิวจาน / Drilled</strong><br />ตัวอย่างจาน Carbon Ceramic ของ AMG ในงาน Geneva Auto Show 2011 ภาพ: <Ref to="amgPhoto">Cheeni</Ref> · <Ref to="ccBySa3">CC BY-SA 3.0</Ref> · แสดงเต็มเฟรม ย่อขนาดสำหรับเว็บ</Photo>
        </div>
        ...
        <Note>คำแนะนำด้านความคุ้มค่าในตารางเป็นการสังเคราะห์ของ <BrandHomeLink /> ภาพใช้ชี้ให้เห็นร่องกับรูบนผิวจาน ไม่ใช่การเทียบสมรรถนะของจานสองรุ่นหรือการรับรองว่าติดตั้งกับ EV ได้ ส่วนจานผิวเรียบดูตัวอย่างได้ที่ <Ref to="prime">Brembo Prime</Ref> และตรวจรหัสที่ตรงรถก่อนเสมอ</Note>
      </Section>
      ...
      <Section id="value" title="6. สาย Value: อัปเกรดตามลำดับที่แก้ปัญหาได้จริง">
        <ol className="space-y-4">
          {[
            ...
            ['04', 'BBK เมื่อมีโจทย์ที่ต้องใช้ความสามารถเพิ่ม', 'Big Brake Kit ควรมีเหตุผลเรื่องแรงบิด ความร้อน ผ้า หรือความแข็งแกร่งของชุด พร้อมตรวจล้อ สมดุลหน้า–หลัง ระบบช่วยเบรก และเบรกมือไฟฟ้า ไม่ใช่ประกอบจากของที่ดูเข้ากัน'],
      ...
      <Section id="japan" title="7. เคสเจ้าของรถ: ENDLESS 6 พอต จาน 375 mm บน Model 3 Performance Highland">
        <p>เจ้าของรถรายงานว่าเคยใช้ ENDLESS 6 พอตกับจานหน้าคู่ <strong>375 mm</strong> บน Tesla Model 3 Performance Highland ต้นทุนคาลิเปอร์และจานคู่หน้าประมาณ <strong>200,000 บาท</strong> เป็นประสบการณ์ซื้อและใช้งานเฉพาะคัน ไม่ใช่ราคาเสนอขายปัจจุบันของ <BrandHomeLink /></p>
        <div className="grid gap-3 sm:grid-cols-3">{[['375 mm', 'จานหน้าตามข้อมูลเจ้าของ'], ['6 พอต', 'คาลิเปอร์ ENDLESS'], ['≈ 200,000 บาท', 'คาลิเปอร์และจานคู่หน้า · เจ้าของรายงาน']].map(([value, label]) => <div key={value} className="rounded-2xl border border-lime-200 bg-lime-50 p-5"><strong className="block text-2xl text-slate-950">{value}</strong><span className="text-sm leading-relaxed text-slate-600">{label}</span></div>)}</div>
        <div className="grid items-start gap-4 sm:grid-cols-2">
          <Photo src="/images/articles/endless-m6-owner-model3-installed.jpg" alt="ภาพที่เจ้าของส่งของคาลิเปอร์ ENDLESS สีน้ำเงินและจาน 375 mm ติดตั้งบน Model 3 Performance Highland" width={2906} height={2581}>ภาพจากเจ้าของรถ: ENDLESS และจานหน้าที่ติดตั้งจริง ขนาด 375 mm เป็นข้อมูลที่เจ้าของยืนยัน ไม่ได้วัดสเกลจากภาพ</Photo>
          <Photo src="/images/articles/endless-m6-owner-front-kit.jpg" alt="คาลิเปอร์ ENDLESS สีน้ำเงินสองตัวและจานเบรกหน้าสองใบพร้อมกล่อง จากภาพเจ้าของรถ" width={2831} height={2153}>ภาพจากเจ้าของรถ: คาลิเปอร์คู่และจานคู่หน้า การระบุรายการที่รวมในราคาต้องอ้างใบเสนอราคาหรือใบเสร็จเพิ่มเติม</Photo>
        </div>
        <p><strong>สภาพรถในเคสนี้:</strong> ล้อ Advan 20×10.5 ทั้งสี่ล้อ ยาง Michelin Pilot Sport 4S โช้ค H-Drive Euro Spec ชิ้นส่วนช่วงล่าง Hardrace หลายรายการ และพาร์ตคาร์บอน ตามข้อมูลเจ้าของ จึงไม่ใช่ผลทดสอบรถสเปกโรงงานหรือการรับรองล้อ 20 นิ้วทุกแบบ</p>
        <p>ฉลากที่เจ้าของส่งระบุ <strong>EC690 และ RCP209 MX72</strong> สอดคล้องกับ <Ref to="endlessM6">ENDLESS M6 ในแคตตาล็อก Global</Ref> ซึ่งเป็นคนละรุ่นกับ ALVELL6 ในตารางราคาญี่ปุ่นด้านล่าง ความหนาจาน รหัสชุดสำเร็จที่ผู้ผลิตรับรองสำหรับ Highland และรายการผ้า/สาย/ขายึด/ภาษี/ค่าแรงที่รวมในยอดประมาณสองแสน ยังเป็น <strong>Data unavailable</strong> ในหลักฐานชุดนี้ จึงไม่แสดงยอดดังกล่าวเป็นราคาครบชุดที่ตรวจยืนยัน หรือใช้คำนวณว่าถูกกว่าชุดอื่นกี่เปอร์เซ็นต์</p>
        <h3 className="text-xl font-bold">ถ้าวัดได้ไม่เกิน 200°C ทำไมต้องจ่ายเพิ่มเพื่อ 700°C?</h3>
        <p>เจ้าของรายงานว่าใช้กล้อง Thermal วัดจากการใช้งานถนนจริงแล้ว <strong>ไม่เคยเห็นค่าเกิน 200°C</strong> ขณะที่ <Ref to="mx72">ENDLESS ระบุช่วงอุณหภูมิจานสำหรับผ้า MX72 ที่ 50–700°C</Ref> ตัวเลข 700°C เป็นขอบเขตของ compound นี้ ไม่ใช่พิกัดความร้อนของเบรกทั้งชุด และไม่ใช่เป้าหมายที่ต้องขับให้ถึง</p>
        <Note>ยังไม่มีรายละเอียดจังหวะอ่านกล้อง จุดวัด ค่าการแผ่รังสีของผิว (emissivity) หรือไฟล์ภาพความร้อน ให้ตรวจ จึงใช้ข้อมูลนี้เป็นประสบการณ์เจ้าของ ไม่เรียกว่าอุณหภูมิสูงสุดระหว่างเบรกที่ยืนยันแล้ว <Ref to="apTemp">AP Racing อธิบายว่าอุณหภูมิลดลงเร็วหลังหยุด</Ref> และไม่ควรคำนวณ 200 ÷ 700 เป็น “ใช้สมรรถนะไปกี่เปอร์เซ็นต์”</Note>
        <p><strong>ข้อเสนอของ <BrandHomeLink />:</strong> ถ้าการใช้งานจริงไม่มีอาการเฟด ระบบสมบูรณ์ และผลตรวจสอดคล้องกับขอบเขตชุด การขยับไปชุดร้อนกว่าเพียงเพื่อเพิ่มตัวเลขอาจไม่ตอบโจทย์ คุณอาจให้ค่าน้ำหนัก ฟีล ฝุ่น รูปลักษณ์ หรืออะไหล่ทดแทนมากกว่า แต่ต้องประเมินจากข้อมูลของรถคันนั้น ไม่สรุปว่าเบรกทุกคันใช้ไม่ถึง 200°C</p>
      </Section>
      ...
      <Section id="complete-kits" title="8. เทียบชุดหน้าแท้มือหนึ่ง: ต้องรู้รหัสและรู้ว่าได้อะไร">
        ...
        <p><strong>ตัวเลขราคาไทยพร้อมใช้:</strong> Data unavailable สำหรับใบเสนอราคาที่ตรวจครบเงื่อนไขทั้งสี่ชุด จึงแสดงราคา USD/JPY จากต้นทาง พร้อมภาษีที่แหล่งข้อมูลระบุ โดยไม่แปลงเป็นยอดขายไทย ต้องรวมค่าขนส่ง นำเข้า ภาษี งานติดตั้ง และความรับผิดชอบหลังขายก่อนตัดสินใจซื้อ</p>
      </Section>

      <Section id="brand-compare" title="9. Brembo / ENDLESS / Carbon Ceramic: แตะวงเพื่อดูคะแนน">
        <p>เริ่มจากสามชื่อที่คุ้นเคย แล้วแตะ Performance ราคา หรือฝุ่นเพื่อเปิดคะแนนคร่าว ๆ พร้อมเหตุผลและลิงก์รีวิว กราฟนี้เป็นมุมมองบรรณาธิการสำหรับการใช้ถนน ส่วนรหัสและราคาครบชุดที่ตรวจได้อยู่ในตารางด้านบน</p>
        <BrakeBrandCompare />
      </Section>

      <Section id="carbon-ceramic" title="10. สายจัดเต็ม: Carbon Ceramic มีเหตุผลให้ชอบ โดยไม่ต้องอ้างว่าทุกคนต้องใช้">
        <div className="space-y-4"><h3 className="text-xl font-bold">สิ่งที่ได้ เมื่อเลือกชุดเหมาะกับรถ</h3><p>CCB ใช้วัสดุคอมโพสิตที่มีเส้นใยคาร์บอนกับเมทริกซ์เซรามิก จุดเด่นคือมวลจานที่ต่ำและการรองรับความร้อนสูงในชุดที่ออกแบบมา รวมถึงรายละเอียดงานที่หลายคนให้คุณค่า</p><p><Ref to="ccb">Brembo ระบุว่าจานคาร์บอนเซรามิกของตนหนักประมาณครึ่งหนึ่งของจานเหล็กหล่อ</Ref> แต่ต้องชั่งชุดที่เปรียบเทียบจริง ไม่เหมาว่าน้ำหนักระบบเบรกทั้งหมดลดครึ่งหนึ่ง และไม่มีสูตรสากลว่าเบาลงใต้สปริง 1 kg เท่ากับลดน้ำหนักตัวรถกี่ kg</p><p>ถ้าคุณต้องการวัสดุระดับนี้และงบพร้อม การเลือก CCB เป็นความชอบที่มีเหตุผลได้ เพียงรวมต้นทุนชิ้นส่วน การตรวจสภาพ และวิธีดูแลไว้ตั้งแต่ก่อนซื้อ</p></div>
        ...
        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 className="text-xl font-bold">150,000–400,000 บาท อาจเป็น “เฉพาะจาน”</h3><p className="mt-3">เจ้าของเคส ENDLESS รายงานว่าเคยได้รับข้อมูลราคาจาน Carbon Ceramic เริ่มประมาณ <strong>150,000–200,000 บาท</strong> และเกรดราคาสูงประมาณ <strong>300,000–400,000 บาท เฉพาะจาน</strong> ไม่ใช่ราคาชุดคาลิเปอร์และอุปกรณ์ครบ</p><p className="mt-3 text-sm text-slate-600">นี่เป็นช่วงราคาที่เจ้าของบอกเล่า ยังไม่มีใบเสนอราคา รุ่นวัสดุ รหัส ขนาด จำนวนจาน อุปกรณ์ดุมกลาง และภาษีให้ตรวจครบ จึงไม่ยืนยันเป็นราคาตลาดหรือราคาผู้ผลิต และไม่นำไปหาส่วนต่างกับราคาชุดหน้าใหม่</p></div>
        <h3 className="text-xl font-bold">ทำไมชุดแพงจึงขายเป็นระบบ มากกว่าแค่จานสองใบ</h3>
        <p><Ref to="plaid">Tesla Model S Plaid Track Package</Ref> ที่อ้างอิงระบุจานหน้า 410 × 40 mm หลัง 410 × 32 mm พร้อมคาลิเปอร์ ผ้า น้ำมัน เบรกมือ และ firmware รวมทั้งข้อกำหนดล้อ ชุด 410 mm นี้อยู่นอกช่วงเปรียบเทียบ 375–390 mm จึงไม่นำราคาไปปนในตารางชุดหน้าใหม่ ใช้เป็นตัวอย่างว่าการจับคู่ระบบสำคัญกว่าการเลือกเส้นผ่านศูนย์กลางหรือวัสดุแยกชิ้น ห้ามย้ายสเปกนี้ไปใช้กับ Model 3 หรือรถไทยรุ่นอื่นโดยไม่มีเอกสารรองรับ</p>
      ...
      <Section id="thai-roads" title="11. ถนนไทยเส้นไหนจะทำเบรกถึง 1,000°C ได้จริง?">
      ...
      <Section id="temperature" title="12. พลังงาน 1 MJ ไม่ได้แปลว่าจานจะร้อนกี่องศา">
      ...
      <Section id="before-buying" title="13. ก่อนจ่ายเงิน ให้ร้านตอบเรื่องเหล่านี้ครบ">
        <ul className="list-disc space-y-3 pl-6 marker:text-lime-700"><li><strong>ตรงรถ:</strong> ปี รุ่นย่อย ตลาด VIN และระบบเบรกเดิม ไม่ใช้แค่ชื่อ Model 3 / Model Y / BYD เป็นคำรับรอง</li><li><strong>ของใหม่และตรวจที่มาได้:</strong> รหัสชุดจากผู้ผลิต ใบซื้อจากตัวแทนที่ตรวจสอบได้ หมายเลขชิ้นส่วน เงื่อนไขรับประกัน และรายการ 2 คาลิเปอร์ / 2 จาน / ผ้า / ขายึด / สาย / อุปกรณ์ให้ครบ ไม่ถือว่าการติดโลโก้พิสูจน์ที่มาแล้ว</li><li><strong>ตรงชุด:</strong> รหัสจาน คาลิเปอร์ ผ้า ความหนาจาน พื้นที่ทำงาน สมดุลหน้า–หลัง และความเข้ากันได้กับ ABS/ESC/เบรกมือ</li><li><strong>ตรงล้อ:</strong> แบบตรวจระยะเคลียร์กับขนาดล้อ ก้านล้อ และออฟเซ็ตจริง ไม่ตัดสินจากเลขนิ้วอย่างเดียว</li><li><strong>ตรงงาน:</strong> ถนนหรือสนาม ใช้เมื่อเย็นได้อย่างไร เสียง/ฝุ่น/การสึกที่ต้องยอมรับ และขั้นตอนปรับหน้าสัมผัสผ้ากับจาน (bedding-in) ของชุดนั้น</li><li><strong>ตรงงบตลอดการใช้:</strong> ค่าอุปกรณ์ ติดตั้ง น้ำมัน ผ้า/จานทดแทน การตรวจสภาพ ค่าขนส่งและระยะรออะไหล่ ขอราคาแยกให้ครบ</li><li><strong>ตรวจส่งมอบ:</strong> รายการตรวจรั่ว ระยะเคลียร์ การทำงานระบบช่วยเบรก และกำหนดตรวจซ้ำที่ผู้ติดตั้งรับผิดชอบ</li></ul>
        <div className="rounded-2xl border border-lime-200 bg-lime-50 p-5"><h3 className="text-xl font-bold">ต้นทุนที่ควรเทียบ</h3><p className="mt-3">ชุดแรก + งานติดตั้งที่จำเป็น + ผ้า/จานที่คาดว่าจะเปลี่ยนตามการใช้งาน + งานตรวจ/บริการ + ค่าอะไหล่เฉพาะชุด</p><p className="mt-2 text-sm text-slate-600">มีราคาชุดใหม่จากต้นทาง USD/JPY แต่ใบเสนอราคาไทยที่ตรวจครบรายการยังเป็น Data unavailable ตัวเลขประมาณสองแสนของเจ้าของรถเป็นคนละชนิดหลักฐานกับราคาผู้ผลิต จึงต้องขอรหัสชุดและรายการรวม/ไม่รวมก่อนเทียบยอดจบ</p><a href="/downloads/evselect-brake-shop-checklist.txt" download className="mt-4 inline-flex min-h-11 items-center gap-2 font-bold text-lime-900 underline underline-offset-4"><Download className="h-5 w-5 shrink-0" aria-hidden="true" />ดาวน์โหลดเช็กลิสต์ภาษาไทย ส่งให้ร้านก่อนซื้อ</a></div>
      </Section>

      <Section id="faq" title="14. คำถามที่คนซื้อเบรกเจอบ่อย"><div className="space-y-3">{faqs.map(item => <details className={styles.detail} key={item.q}><summary>{item.q}</summary><p className="text-base text-slate-600">{item.a}</p></details>)}</div></Section>

      <section id="sources" aria-labelledby="sources-title" className="rounded-2xl bg-slate-50 p-5 text-sm sm:p-7"><h2 id="sources-title" className="mb-4 text-xl font-bold">ที่มาและขอบเขตของบทความ</h2><p className="mb-4">ตรวจแหล่งข้อมูลวันที่ 19 กันยายน 2569 บทความนี้เป็นการสังเคราะห์เอกสารผู้ผลิตและการคำนวณตัวอย่างของ <BrandHomeLink /> ไม่ใช่ผลทดสอบเบรกบนถนนไทย <BrandHomeLink /> ไม่ได้วัดระยะหยุด ฝุ่น หรืออุณหภูมิจากรถจริงในงานนี้ ภาพและข้อมูล Model 3 Performance Highland เป็นข้อมูลที่เจ้าของส่งพร้อมยืนยันจาน 375 mm แยกจากราคาแคตตาล็อกและผลการทดสอบของผู้ผลิต คะแนนในกราฟเป็นการประเมินคร่าว ๆ โดย <BrandHomeLink /> มีรีวิวผู้ใช้ต่างรถและต่างปีประกอบในช่องเหตุผล ไม่ใช่คะแนนเฉลี่ยจากลูกค้าหรือผลทดสอบมาตรฐาน ข้อมูลผลิตภัณฑ์และการรองรับรถอาจเปลี่ยนตามปี รุ่น และตลาด</p><ul className="list-disc space-y-2 pl-5">{Object.entries(brakeSources).map(([key, source]) => <li key={key}><a href={source.url} target="_blank" rel="noopener noreferrer" className="text-slate-600 underline underline-offset-4 hover:text-lime-800">{source.title}</a></li>)}</ul></section>
